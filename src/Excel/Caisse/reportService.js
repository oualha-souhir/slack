...
async function analyzeTrends(context) {
	...
	try {
		...
		const summarizedOrders = orders.map((order) => ({
			...
			status: order.statut,
			totalAmount:
				order.proformas?.reduce((sum, p) => sum + (p.montant || 0), 0) || 0,
...
📊 **Distribution des statuts:** ${Object.entries(
...
${
	trends.anomalies?.length
		? `⚠️ **Anomalies:** ${trends.anomalies.join(", ")}`
		: ""
}
${
	trends.recommendations?.length
		? `💡 **Recommandations:** ${trends.recommendations.join(", ")}`
		: ""
}`;
		...
		try {
			...
			await notifyTechSlack(notificationError);
