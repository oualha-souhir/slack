...
const { createSlackResponse } = require("./slackUtils");
const { getEquipeOptions } = require("../Configurations/config");
...
async function suggestAutoCompletions(userId, context) {
	...
	const pastOrders = await Order.find({ demandeur: userId })
		.sort({ date: -1 })
		.limit(5);
  ...
  `;
	...
	try {
		...
		try {
			...
			await notifyTechSlack(jsonError);
