...
async function checkPendingPaymentRequestDelays(context) {
	try {
		...
		if (pendingPaymentRequests.length > 0) {
			let summary = `:warning: *Rappel quotidien - demande de décaissement en attente*\n\n`;
			summary += `*Demande de décaissement non traitées depuis plus de 24h:*\n`;
...
async function checkPaymentRequestApprovalDelays(context) {
	try {
		...
		for (const [channelId, requests] of PaymentRequestByChannel) {
			let summary = `:warning: *Rappel quotidien - demande de décaissement en attente*\n\n`;
			summary += `*Demande de décaissement validées sans paiement depuis plus de 24h:*\n`;
